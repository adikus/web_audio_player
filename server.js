...
function retrieveTrackInfo(id, reload, cb) {
    ...
    exec('lib/yt-dlp -j -- ' + id, {maxBuffer: 1024 * 1024}, function (error, stdout, stderr){
        if(stdout.length > 2){
...
app.get('/yt-playlist/:id/info', function(req, res) {
    ...
    exec('lib/yt-dlp ' + additionalParams + ' -j --flat-playlist -- ' + req.params.id, {maxBuffer: 1024 * 1024}, function (error, stdout){
